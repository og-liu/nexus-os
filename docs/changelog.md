...
### 根因
- 顶部筛选栏的标签计数吃 `listTags`，而它数的是关联表 `knowledge_item_tags` 里「标签出现次数」，**不筛条目状态**。
- 知识流列表却只显示 `status=kept`（留下的）。于是回收站 / 待处理 / 丢弃里打过同名标签的条目，也被算进计数，导致数字虚高。

### 修复
- `listTags` 加可选 `status` 过滤：传了状态就 `JOIN knowledge_items` 只数该状态下的条目，不传保持全量（向后兼容，调用方零改动）。
- `GET /api/knowledge/tags` 返回拆成两组：`tags`（全量，驱动打标签候选 + 「管理标签」列表，标签只要出现过就该可见）与 `feedTags`（只数 kept，驱动顶部筛选栏计数，跟列表一条不差）。
- 前端新增 `feedTags` state：顶部筛选栏的渲染与「展开 +N」计数改吃 `feedTags`；重命名 / 删除标签时同步更新 `feedTags`，避免旧名残留。

---

## 2026-08-27(下午·五) — 知识流「减法 + 标签多选筛选」改造

用户对知识流做了一轮「减负 + 收口」，一句话概括：**别让用户管「读没读」，只让他看「处理没处理」**；筛选收口到顶部标签，卡片上的标签降级为纯样式标识。

### 彻底移除「已读/未读」机制
- 判定：这个状态对用户没价值（他要的是「有没有保留/放弃」，不是「有没有点开过」），蓝点样式也一直没生效，干脆整条链路砍掉。
- 拆除三层、一处不留：数据库 `read_at` 列（CREATE TABLE 定义 + 增量迁移分支 + 存量库 `ALTER TABLE ... DROP COLUMN read_at`）；服务层 `markRead` / `countUnread` 函数、`ListOptions.unreadOnly` / `read?` patch、列表 SELECT 里的 read_at 列；前端 `unread` 字段、蓝点、侧栏未读角标、「只看未读」开关、回顾页「从未读过」标记、打开详情即标已读的逻辑。
- 连带两处口径跟着改：侧栏「待处理」角标从「未读的待处理数」改成「待处理总数」（= status=inbox 的条目数，纯「没操作过」）；回顾候选排序从「未读优先」改成「创建时间最久优先」（存得越久越该重温）。

### 顶部标签多选筛选
- 砍掉「全部/未读/近七天」三档 chips（`feedFilter`），改成顶部平铺所有标签，命中逻辑 = 多标签 AND 交集。
- 点选/再点取消，空选 = 全部；标签按频次降序排，超过 12 个默认折叠（「展开 +N」展示全量）；全设备 `flex-wrap` 换行，手机端不横滑。
- 卡片上的标签 pill 从「点击即筛选」降级为纯样式标识——筛选统一收口到顶部，一处入口，省得满屏找。
- 服务层 `listItems` 的 `tag`（单值）升级成 `tags`（数组），多标签各拼一个 `EXISTS` AND 子句；`GET /api/knowledge` 改用 `sp.getAll("tag")` 支持重复 tag 参数。

### 视觉：方角统一
- 顶部筛选 chips、卡片/注记/回顾卡的标签 pill、标签勾选区，`rounded-full` 椭圆统一改成 `rounded-[2px]` 方角，对齐搜索框的方正感。
- 圆形语义保留：圆点、加号按钮、角标红点——这些本来就是「圆」，不是「椭圆按钮」。

### 测试与文档
- `store.test.ts`：删掉依赖 read_at 的「从未读排最前」用例，改成「创建最久排最前」的断言；`tag` 单值参数同步改成 `tags` 数组；删掉已废弃的 `since` 过滤用例。

---

## 2026-08-27(下午·四) — 验收反馈修复：列表不刷新 bug + 「知识库」改名「知识流」

### 加入/移出后列表不刷新（bug）
「我的文章」里加入知识流后，切到知识流看到的还是旧列表，要手动刷新才出现。根因：`addNoteToLibrary` / `removeNoteFromLibrary` / `batchAddToLibrary` 成功后只更新了「我的文章」的 inLibrary 标记，而知识流列表（feed）只在首屏加载时拉过一次。反向操作（知识流里移出 → 重拉文章列表）当时做了，正向漏了对称的一步。修复：三个函数成功路径补 `loadFeed`（带当前搜索词/标签/筛选器，保持用户正在看的视图口径一致），批量版与 loadNotes 并行重拉。

### 「知识库」全局改名「知识流」
用户拍板把产品命名统一回「知识流」。范围不止 UI：侧栏 tab「我的知识库」→「知识流」、「加入/移出知识库」按钮与 toast、搜索 placeholder、空态文案、重复检测提示「已在「知识流」里」，还包括 **AI 对话的系统提示词和 Agent 工具描述**（模型转述给用户时措辞才不会串台）。改完 `src/` 内 grep「知识库」零残留；changelog 历史记录保留原文（记录当时的事实）。代码标识符（knowledge 路由 / store / 工具名）不动——改的是用户看到的名字，不是数据层的地基。

---

## 2026-08-27(下午·三) — 知识模块重构·阶段4 P2：摘录 / 每日回顾 / 智能列表 / OPML 导入导出 / 自动归档规则 / 过期清理提示 / 批量去重

阶段 1-3 把「存得进、读得快、标得准」打通后，阶段 4 是知识的「长线经营」：让存下的东西反复产生价值，让整理成本趋近于零。方案里的可选项（P2）这轮全部落地，外加阶段 2 挂起的批量去重 UI。

### 摘录（划线存话）
- **做减法**：只做「存住 / 回看 / 能删」的纯文本摘录，不做原文内高亮渲染。原因：详情页正文是净化后的 HTML 快照，用户选区跨节点时没有稳定的 DOM 定位方式，硬做高亮要存偏移量并和快照版本绑死——为锦上添花引入版本耦合不值当，先让核心价值成立。
- 详情页正文 `onMouseUp`（不监听 selectionchange——选区事件太吵）+ `setTimeout(0)` 等浏览器先结算选区；选中 3-1000 字出现「存为摘录」提示条，摘录列表黑左线灰底展示、hover 可删。
- **幂等设计**：同一篇文章同一句话摘两次不报错不重复（duplicate 返回 200）；入库前空白压平（`\s+` → 单空格），换行缩进不污染数据。
- note（自己的文章）分支不做摘录——自己写的本来就是全文在手。

### 每日回顾（真实数据替换原型假数据）
- 原型期 review tab 的「今日采集 8 / 留存 5 / 技术晨报已生成」全是写死的假数据，本轮全部换成 `/api/knowledge/review` 真实统计。
- **「重温旧藏」排序**：`COALESCE(read_at, 0) ASC` 让从未读过的排最前（「存了不读」是个人库的头号病灶），读过的按最早阅读时间——越久没见的越先回来。取 20 条 Fisher-Yates 洗牌后取 4，避免固定「最老四条」造成疲劳；「换一批」重拉即新组合。
- **todayKept 用 updated_at 近似拍板时间**：个人库改标签会轻微虚高但方向正确，等真实反馈再决定是否单独立拍板时间戳列。

### 智能列表（知识库快捷过滤）
feed 视图加 chips：全部 / 未读 / 近七天。走服务端过滤（`since` 参数进 ListOptions，created_at >=），和搜索词、标签共用同一条 350ms 防抖通道——一处节流逻辑管全部触发源。

### OPML 导入导出
- 挂「自动」页（订阅源管理区旁边）：自动化的事都在自动页，语义统一。
- **解析用正则不用 XML 库**：只需要 outline 的 xmlUrl / title / text 三个属性，为读三个属性引入整棵 XML 解析器不值当；不假设属性顺序、还原 HTML 实体、同文件内去重。
- **导入不自动抓取**：一份 OPML 几十个源逐个抓要几分钟必超时；入库完成后由既有的整点自动抓兜底（单个手动添加才有「立即见货」的即时感，批量导入的语义是「先到位」）。已存在的源自动跳过。
- 导出带 `Content-Disposition` + RFC5987 编码的中文文件名，浏览器直接下载。

### 自动归档规则（只打标，不拍板）
- **铁律：规则只打标签不替人做留弃决定**。打标是整理（可逆、无损），拍板是决策（丢弃有反悔成本）——这是本模块自始至终的边界，自动化的舒适区在「把素材摆到正确的抽屉」，不在「替你扔东西」。
- 两型规则：**链接域名**（hostname 精确匹配，如 `github.com`）和**标题正文含**（keyword，小写包含匹配，正文只看前 2000 字——文章主旨在开头，也控制匹配成本）。
- **挂点**：手动采集（URL 抓取成功 / 降级分支降级时 keyword 规则对 URL 落条目不生效，等 refetch 重抓后由 AI 候选标签兜底 / 手写文章不挂——note 语义）+ RSS ingest 落库后同步执行（纯本地 SQL 几毫秒，不像 AI 解读要等 LLM，无 await 不破坏「查重到 INSERT 间不许插 await」的并发保护）。md 导入不挂（note/draft 语义，与不自动解读一致）。
- 命中即**追加**标签不覆盖——人工打的标签永远优先。规则管理在自动页：类型徽标 + `pattern → tag` 胶囊 + 停用/启用/删除。已有条目不追溯打标（规则生效从入库那一刻开始）。

### 过期清理提示（只提示，不自动删）
- 待处理里躺超 30 天（`KNOWLEDGE_INBOX_AGING_DAYS` 可调）的条数进 counts.agedInbox，inbox 列表顶部出现提示条「有 N 条在待处理里躺了超过 30 天」+「都不要了」一键批量 discard（前端仍按创建时间过滤校验 + confirm 二次确认「先进不要了可反悔」）。
- **不自动删除**：提示无破坏性所以默认开（`KNOWLEDGE_INBOX_AGING=0/off/false` 可关）；一键丢弃进的是「不要了」状态而非物理删除，反悔路保留。自动删除是无人监督下的不可逆决策，不做。

### 批量去重（阶段 2 挂起项补齐）
- feed 视图加「查重」按钮，进入重复报告视图：按组展示（原因标签「同一链接存了多次」/「内容几乎一样」），每组「保留最新，丢弃其余」——最新的信息最全；组内条目标注「将保留 / 将丢弃」。
- 复用阶段 2 已就绪的 findDuplicates（URL 归一化 + SimHash 双层检测）；批量丢弃走 discarded 状态非物理删——批量操作误判空间大，必须留反悔路。查重中可随时退出回常规列表。

### 技术细节
- 新表 `knowledge_excerpts`（FK ON DELETE CASCADE，条目删摘录自动跟着删）和 `knowledge_rules`（type CHECK 约束在 domain/keyword 二选一）。
- 新 API 路由 5 个：`[id]/excerpts`（GET/POST/DELETE）、`rules`（GET/POST/PATCH/DELETE，id 走 query——静态段路由优先于 [id] 动态段不会冲突）、`duplicates`（GET 报告 / POST 批量丢弃）、`review`（GET 回顾数据）、`feeds/opml`（GET 导出 / POST 导入）。
- 单测 15 例：excerpts 4（正序读回/幂等/长度拒绝/删除）+ rules 7（含 2000 字边界、追加不覆盖、停用安全、人工标签自由改）+ store 4（since 过滤/getReviewItems 排序/只有 kept/countAgedInbox）。
- review 的 revisit 列表用 tagMap 单独 IN 查询挂标签（attachTags 要完整行数组，回顾只需要 4 个字段，不为此拖全量列）。
