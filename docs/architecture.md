...
2. **AI 对话**（已实现）：用户输入 → `POST /api/chat` → 归档残留中断任务 → 组装上下文（滑动窗口 20 轮）→ Agent 编排（简单消息直答；任务型消息走 Plan-and-Execute：规划拆步 → 逐步执行[每步小型 ReAct+失败重试] → 汇总；支持 HITL 补问暂停-续跑与断点恢复）→ SSE 流式输出（delta / reasoning / tool_* / plan_* / step_*，结束时带 usage）→ SQLite 增量落库（正文、工具调用 JSON、思考全文、token 用量、计划状态，停止/刷新可还原半截内容）
