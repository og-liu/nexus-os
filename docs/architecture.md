...
2. **AI 对话**（已实现）：用户输入 → `POST /api/chat` → 崩溃残留自愈（孤儿 running 计划/占位消息翻 stopped）→ 会话并发锁（同会话已有轮次在跑则 409）→ 归档残留中断任务 → 组装上下文（滑动窗口 20 轮，rowid 保证同毫秒消息顺序稳定）→ Agent 编排（简单消息直答；任务型消息走 Plan-and-Execute：规划拆步 → 逐步执行[每步小型 ReAct+失败重试] → 汇总；支持 HITL 补问暂停-续跑与断点恢复）→ SSE 流式输出（delta / reasoning / tool_* / plan_* / step_*，结束时带 usage）→ SQLite 增量落库（正文、工具调用 JSON、思考全文、token 用量、计划状态，停止/刷新可还原半截内容）
...
- AI Agent 工具调用、思考过程与 token 用量持久化（messages 表 tool_calls / reasoning / usage 字段，幂等迁移，重开会话还原）
- AI Agent 任务规划 Plan-and-Execute（planner 拆步 ≤8 步 → 逐步执行 → 失败重试 → 流式汇总；简单直答绕开规划）
- HITL 补问暂停-续跑（ask_user 步骤暂停抛问，用户回复后断点续跑，已完成步骤结果回灌）
- 真停止与断点恢复（AbortSignal 贯穿 + 占位行增量落盘 800ms 节流；stopped 计划支持继续/放弃，整轮配对归档杜绝旧任务污染上下文）
- Vitest 单元测试体系（内存 SQLite 测试库，7 用例覆盖归档与计划状态流转）；Node 版本统一 22.23.2（.nvmrc）
...
- 工具实际执行逻辑
- 文件管理功能
- AI Agent 意图理解深化（关键词启发式判定简单/任务型消息；历史压缩、关键信息提取待做）
- 知识库数据存储、采集、AI 整理与语义检索（含 Agent 的知识库优先问答衔接）
- 自动化工作流引擎
- 插件系统
