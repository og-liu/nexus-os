...
2. **AI 对话**（已实现）：用户输入 → `POST /api/chat` → 组装上下文（滑动窗口 20 轮）→ Agent Loop（流式调模型 → 执行工具 → 回传再决策，上限 5 轮）→ SSE 流式输出（含 tool_call / tool_result / tool_error，结束时带 usage）→ SQLite 落库（正文 + 工具调用 JSON + 思考全文 + token 用量，重开会话可还原）
