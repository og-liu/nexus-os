...
| AI Agent | `/agent` | 知识库优先问答（RAG）、意图理解、任务规划与工具调用 |
| 工具中心 | `/tools` | 图片处理、文件批处理、OCR、格式转换等效率工具 |
| 文件管理 | `/files` | 本地文件浏览、分类、搜索与管理 |
| 知识库 | `/knowledge` | 采集 → 审查 → AI 整理归档 → 阅读检索的知识流水线 |
| 自动化 | `/automation` | 定时采集综述、智能复习、工作流编排与定时任务 |
...
- [ ] 文件管理：本地文件浏览与操作
- [ ] AI Agent：知识库优先问答（RAG）、对话界面与工具调用
- [ ] 知识库：采集 → 审查 → AI 整理 → 语义检索
